...
class ShowDotProduct(MovingCameraScene):
    ...
    def get_product(self, v1, v2):
        return np.dot(v1.get_vectors(), v2.get_vectors())
...
class ShowCrossProduct(ShowDotProduct):
    ...
    def get_product(self, v1, v2):
        return get_norm(
            np.cross(v1.get_vectors(), v2.get_vectors())
        )
...
class DivergenceTinyNudgesView(MovingCameraScene):
    ...
    def take_tiny_step(self):
        ...
        self.play(
            ...
            ReplacementTransform(
                new_vector,
                diff_vector.copy().shift(-vector.get_vectors()),
                remover=True
            ),
            diff_vector.shift, -vector.get_vectors(),
    ...
    def show_dot_product(self):
        ...
        def update_moving_diff_vector(dv):
            step = moving_step_vector.get_vectors()
            o1 = self.vector_field.get_vector(point).get_vectors()
            o2 = self.vector_field.get_vector(point + step).get_vectors()
            diff = o2 - o1
        ...
        dot_product_value_update = ContinualChangingDecimal(
            dot_product_value,
            lambda a: np.dot(
                moving_step_vector.get_vectors(),
                moving_diff_vector.get_vectors(),
            ),
            **dp_kwargs
        )
        ...
        for s in 0.6, -0.6:
            for step, diff in zip(all_step_vectors, all_diff_vectors):
                ...
                diff.target.put_start_and_end_on(
                    step.get_end(),
                    step.get_end() + s * step.get_vectors()
                )
            self.play(
